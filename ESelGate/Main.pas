...
type
  TUserSession = record
    ...
    nReviceMsgLength: Integer; //3秒内接受数据总长
    dwReceiveMsgTick: LongWord; //检测TICK
  end;
...
begin
  if ServerSocket.Active then begin
    if (GetTickCount - m_dwAttackTick > 1000 * 60 * 3) and (m_nAttackCount >= 10) then begin
      m_nAttackCount := 0;
      BlockMethod := UseBlockMethod;
      nAttackLevel := nUseAttackLevel;
      if TempBlockIPList <> nil then begin
        TempBlockIPList.Lock;
        try
          for I := 0 to TempBlockIPList.Count - 1 do begin
            if TempBlockIPList.Items[I] <> nil then
...
begin
  ...
  try
    ...
    for nSocketIndex := 0 to GATEMAXSESSION - 1 do begin
      ...
      {if (nAttackLevel > 0) and ((GetTickCount - g_SessionArray[nSocketIndex].dwReceiveMsgTick) > 1000 * nAttackLevel) and (g_SessionArray[nSocketIndex].nReviceMsgLength > 500) then begin
        sRemoteIPaddr := g_SessionArray[nSocketIndex].sRemoteIPaddr;
        Inc(m_nAttackCount);
        m_dwAttackTick := GetTickCount;
        if m_nAttackCount >= 3 then begin
        ...
        if g_SessionArray[nSocketIndex].Socket <> nil then
          g_SessionArray[nSocketIndex].Socket.Close;
        Continue;
      end;}
